//constantes
const NBLIG = 3; //nombre de lignes à afficher
const NBCOL = 6; //nombre de colonnes à afficher
...
function init()  //fonction appelée par le body
{
	...
	soundManager.play("fond","musiques/corpse_bride.mp3");//on met une musique de fond
...
function choisirCarte(event)
{
	...
	for (var i = 0; i < NBLIG*NBCOL; i ++)
	{
		...
		)
		{
			...
			if(nbPairesTrouvees == 9)//si on a trouvé toutes les paires
			{
...
{
	for (var i = 1; i <= 5; i ++)//les premières cartes ont juste l'axe x de modifié
	{
		plateau[i].posX += i*(CARTEW + DX);
	}
	
	for (i = 6; i <= 11; i ++)//même axe y pour toutes, puis axe x différent pour chacune des cartes
	{
		plateau[i].posY = CARTEH + 2*DY;
	}
	plateau[7].posX += CARTEW + DX;
	plateau[8].posX += 2*(CARTEW + DX);
	plateau[9].posX += 3*(CARTEW + DX);
	plateau[10].posX += 4*(CARTEW + DX);
	plateau[11].posX += 5*(CARTEW + DX);
	
	for (i = 12; i <= 17; i ++)//idem
	{
		plateau[i].posY = 2*CARTEH + 3*DY;
	}
	plateau[13].posX += CARTEW + DX;
	plateau[14].posX += 2*(CARTEW + DX);
	plateau[15].posX += 3*(CARTEW + DX);
	plateau[16].posX += 4*(CARTEW + DX);
	plateau[17].posX += 5*(CARTEW + DX);
