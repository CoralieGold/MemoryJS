...
const NBCOL = 4; //nombre de colonnes à afficher
...
function init()  //fonction appelée par le body
{
	...
	soundManager.play("fond","musiques/nightmare_before_christmas1.mp3");//on met une musique de fond
...
function choisirCarte(event)
{
	...
	for (var i = 0; i < NBLIG*NBCOL; i ++)
	{
		...
		)
		{
			...
			else if(secondeCarte == undefined)//sinon, on regarde si la seconde carte n'est pas encore choisie
			{
				...
				if (carte != premiereCarte)//si la première et la seconde cartes sont différentes
				{
					...
					else
					{
						...
						secondeCarte.trouve = true;//on dit que les deux cartes ont été trouvées
			...
			if(nbPairesTrouvees == 6)//si on a trouvé toutes les paires
			{
...
{
	for (var i = 1; i <= 3; i ++)//les premières cartes ont juste l'axe x de modifié
	{
		plateau[i].posX += i*(CARTEW + DX);
	}
	
	for (i = 4; i <= 7; i ++)//même axe y pour toutes, puis axe x différent pour chacune des cartes
	{
		plateau[i].posY = CARTEH + 2*DY;
	}
	plateau[5].posX += CARTEW + DX;
	plateau[6].posX += 2*(CARTEW + DX);
	plateau[7].posX += 3*(CARTEW + DX);
	
	for (i = 8; i <= 11; i ++)//idem
	{
		plateau[i].posY = 2*CARTEH + 3*DY;
	}
	plateau[9].posX += CARTEW + DX;
	plateau[10].posX += 2*(CARTEW + DX);
	plateau[11].posX += 3*(CARTEW + DX);
}
